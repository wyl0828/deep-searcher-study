...
class TestNaiveRAG(BaseAgentTest):
    ...
    def test_retrieve_merges_explicit_queries_deterministically(self):
        self.naive_rag.collection_router.invoke = MagicMock(return_value=(["test_collection"], 0))

        results, tokens, metadata = self.naive_rag.retrieve(
            "比较两份资料",
            retrieval_queries=("比较两份资料", "第一份资料", "第二份资料"),
            top_k=3,
        )
