...
class TestCollectionRouter(BaseAgentTest):
    ...
    def setUp(self):
        ...
        self.collection_router = CollectionRouter(llm=self.llm, vector_db=self.vector_db, dim=8)
    ...
    def test_init(self):
        ...
        self.assertEqual(self.collection_router.all_collections, ["books", "science", "news"])
    ...
    def test_invoke_with_multiple_collections(self):
        ...
        self.llm.chat = MagicMock(
            return_value=ChatResponse(content='["science", "news"]', total_tokens=10)
        )
    ...
    def test_invoke_with_empty_response(self):
        ...
        self.llm.chat = MagicMock(return_value=ChatResponse(content="[]", total_tokens=5))
        ...
        self.assertTrue(self.collection_router.last_decision["fallback_used"])
        self.assertEqual(
            self.collection_router.last_decision["reason"],
    ...
    def test_invoke_with_single_collection(self):
        ...
        single_collection = [
            CollectionInfo(collection_name="single", description="The only collection")
        ]
    ...
    def test_invoke_with_no_description(self):
        ...
        self.llm.chat = MagicMock(
            return_value=ChatResponse(content='["with_desc"]', total_tokens=5)
        )
    ...
    def test_hallucinated_collection_is_filtered_before_search(self):
        self.llm.chat = MagicMock(
            return_value=ChatResponse(
                content='["science", "invented_private_collection"]',
                total_tokens=7,
            )
        )

        selected_collections, tokens = self.collection_router.invoke("question", dim=8)

        self.assertEqual(selected_collections, ["science", "books"])
        self.assertEqual(tokens, 7)
        self.assertNotIn("invented_private_collection", selected_collections)
        self.assertEqual(
            self.collection_router.last_decision["rejected"],
            ["invented_private_collection"],
        )
        self.assertEqual(
            self.collection_router.last_decision["reason"],
            "hallucinated_collections_filtered",
        )

    def test_allowed_collections_are_applied_before_model_routing(self):
        self.llm.chat = MagicMock()
        selected_collections, tokens = self.collection_router.invoke(
            "question",
            dim=8,
            allowed_collections=["science"],
        )

        self.assertEqual(selected_collections, ["science"])
        self.assertEqual(tokens, 0)
        self.llm.chat.assert_not_called()

    def test_explicit_scope_never_falls_back_to_a_broader_collection(self):
        selected_collections = self.collection_router.resolve_explicit(
            ["science", "books", "invented"],
            dim=8,
            allowed_collections=["science"],
        )
        ...
        self.assertFalse(self.collection_router.last_decision["fallback_used"])

    def test_malformed_model_output_uses_safe_available_fallback(self):
        self.llm.chat = MagicMock(return_value=ChatResponse(content="not a list", total_tokens=4))
        self.llm.literal_eval = MagicMock(side_effect=ValueError("invalid"))

        selected_collections, tokens = self.collection_router.invoke("question", dim=8)

        self.assertEqual(selected_collections, ["books"])
        self.assertEqual(tokens, 4)
        self.assertTrue(self.collection_router.last_decision["fallback_used"])
        self.assertEqual(
            self.collection_router.last_decision["reason"],
            "collection_output_parse_failed",
        )
