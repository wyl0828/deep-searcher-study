...
class TestPDFLoader(unittest.TestCase):
    ...
    def test_load_pdf_file(self, mock_pdf_open):
        ...
        # Load the PDF file
        documents = PDFLoader(ocr_enabled=False).load_file(self.pdf_file_path)

        # Verify pdfplumber.open was called
        mock_pdf_open.assert_called_once_with(self.pdf_file_path)

        # Keep one Document per PDF page so citations can preserve real page numbers.
        ...
        for document in documents:
            ...
            self.assertEqual(document.metadata["display_name"], "test.pdf")
