...
class TestAzureSearch(unittest.TestCase):
    ...
    def test_insert_data(self):
        ...
        test_docs = [
            {"text": "hello world", "vector": rng.random(d).tolist(), "id": "doc1"},
            {"text": "hello azure search", "vector": rng.random(d).tolist(), "id": "doc2"},
        ]
    ...
    def test_search_data(self):
        ...
        mock_results.results = [
            {"content": "hello world", "id": "doc1", "@search.score": 0.95},
            {"content": "hello azure search", "id": "doc2", "@search.score": 0.85},
        ]
        ...
        results = azure_search.search_data(collection="test-index", vector=query_vector, top_k=2)
        ...
        for result in results:
            ...
            self.assertEqual(result.metric_type, "AZURE_RELEVANCE")
            self.assertEqual(result.score_kind, "rank_score")
            self.assertTrue(result.higher_is_better)
        self.assertEqual([result.rank_score for result in results], [0.95, 0.85])
    ...
    def test_clear_db(self):
        ...
        mock_client.search.return_value = [{"id": "doc1"}, {"id": "doc2"}]
